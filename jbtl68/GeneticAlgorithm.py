...
for i in range(max_iter):
    ...
    for j in range(pop_size):
        ...
        crossover_point = num_cities-1
        child1 = parents[0][:crossover_point] + parents[1][crossover_point:]
        child2 = parents[1][:crossover_point] + parents[0][crossover_point:] 

        # find the missing cities for each child
        missing_cities1 = set(range(num_cities)) - set(child1)
        missing_cities2 = set(range(num_cities)) - set(child2)

        # replace duplicate cities with missing cities
        for i in range(num_cities):
            if child1.count(child1[i]) > 1:
                child1[i] = missing_cities1.pop()
            if child2.count(child2[i]) > 1:
                child2[i] = missing_cities2.pop()
